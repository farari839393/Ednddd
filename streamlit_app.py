...
def get_task_id_from_url():
    ...
    return query_params.get("task_id", [None])[0]
...
# Check if we have a task ID in URL
url_task_id = get_task_id_from_url()

if url_task_id and not st.session_state.automation_started:
    # If URL has task ID but automation not started, show status page
    st.session_state.task_id = url_task_id
    st.session_state.automation_started = True
    automation_status()
elif st.session_state.automation_started:
    automation_status()
else:
    main_form()
