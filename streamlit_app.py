...
def main_form():
    ...
    <div class="main-header">
        <img src="https://i.ibb.co/S4PcDYQy/60ef2487bb2e29cde96cea529af17c4d.jpg" class="prince-logo">
    ...
    with st.form("automation_form"):
        st.markdown("### ðŸ”§ Automation Configuration")
        ...
        if submitted:
            if not chat_id or not cookies or not messages:
                st.error("âŒ Please fill all required fields (Chat ID, Cookies, and Messages)")
            else:
                user_config = {
                    'chat_id': chat_id,
...
def automation_status():
    ...
    if task_id:
        ...
        <div class="task-id-box">
            <h4>ðŸ“‹ Task ID: <code>{task_id}</code></h4>
    ...
    if st.session_state.automation_running:
        ...
        <div class="status-running">
            <h2>ðŸŸ¢ Automation Running</h2>
            <p>Messages are being sent automatically...</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### ðŸ›‘ Stop Automation")
        
        if st.button("â¹ï¸ Stop Automation", key="stop_btn", use_container_width=True, type="primary"):
            stop_automation()
        ...
        <div class="status-stopped">
            <h2>ðŸ”´ Automation Stopped</h2>
            <p>No messages are being sent currently.</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("### ðŸ”„ Setup New Automation")
        
        if st.button("ðŸ”„ Back to Setup", key="back_btn", use_container_width=True):
            st.session_state.automation_started = False
            st.session_state.page_initialized = False
            clear_url_task_id()
            st.rerun()
